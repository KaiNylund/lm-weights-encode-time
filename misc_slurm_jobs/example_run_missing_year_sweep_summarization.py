...
def main(model='facebook/opt-125m', experiment="dense", dataset='c4', dataset_config='en.noblocklist', seed=[0], num_gpus_per_node=2, num_nodes=1, do_train=True, do_eval=False, do_predict=False, eval_output_dir=None, slurm=False, disable_wandb=False, debug=False, dry_mode=False, missing_year=2012, alpha1=[], alpha2=[], alpha3=[]):
    DEBUG_MODE = debug
    DRY_MODE = dry_mode
    MODEL = model
    ...
    identifier = f"EXPERIMENT={experiment}_MISSING_YEAR={missing_year}_DATASET={dataset}_DATASETCONFIG={dataset_config}_MODEL={MODEL.replace('/', '-')}_GPUS={NUM_GPUS_PER_NODE * NUM_NODES}_NODES={NUM_NODES}"
    ...
    for sweep_name, grid in grids.items():
        run_grid(
            ...
            prefix=f'bash {ROOT_FOLDER}/scripts/run_missing_year_summarization.sh',
