...
"""
Fine-tuning the library models for sequence to sequence summarization.
"""
...
def main():
    ...
    dataset_columns = summarization_name_mapping.get(data_args.dataset_name, ("text", "summary"))
    ...
    if training_args.do_eval:
        ...
        # Try computing metrics 10 times with 1 minute in between attempts
        # if it fails.
        for attempts in range(5):
            try:
                ...
                logger.info("Exception occurred while computing metrics, retrying in 1 minute...")
                time.sleep(60)
            else:
        ...
        # metrics['eval_rouge1f'] = np.mean([x.fmeasure for x in metrics['eval_rouge1']])
        # metrics['eval_rouge2f'] = np.mean([x.fmeasure for x in metrics['eval_rouge2']])
        # metrics['eval_rougeLf'] = np.mean([x.fmeasure for x in metrics['eval_rougeL']])
        # del metrics['eval_rouge1']
        # del metrics['eval_rouge2']
        # del metrics['eval_rougeL']
        # del metrics['eval_rougeLsum']
